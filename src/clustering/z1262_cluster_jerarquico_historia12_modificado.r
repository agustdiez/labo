...
# Importo el archivo de cluster de bajas

setwd( "D:/OneDrive/Personal/Maestria Data Mining/economia_finanzas/exp/CLU1262" )  #cambiar por la carpeta local

dataset = read.delim("cluster_de_bajas.txt", stringsAsFactors = TRUE, sep = "\t")
...
# Specify data frame
resumen_variables <- dataset%>%
group_by(cluster2) %>%                        
summarise_at(vars("ctrx_quarter", "cpayroll_trx", "mcaja_ahorro", "mtarjeta_visa_consumo", "ctarjeta_visa_transacciones",
                     "mcuentas_saldo", "mrentabilidad_annual", "mprestamos_personales", "mactivos_margen", "mpayroll",
                     "Visa_mpagominimo", "Master_fechaalta", "cliente_edad", "chomebanking_transacciones", "Visa_msaldopesos",
                     "Visa_Fvencimiento", "mrentabilidad", "Visa_msaldototal", "Master_Fvencimiento", "mcuenta_corriente",
                     "Visa_mpagospesos", "Visa_fechaalta", "mcomisiones_mantenimiento", "Visa_mfinanciacion_limite",
...
#Edad promedio
edad_promedio <- dataset%>%
group_by(cluster2) %>%                        
summarise_at(vars("cliente_edad","cliente_antiguedad","cliente_vip"),
              list(name = mean))   


cantidad_cluster <- dataset%>% 
       group_by(cluster2) %>% 
       summarise(total_count=n())                     
  
setwd( "D:/OneDrive/Personal/Maestria Data Mining/economia_finanzas/exp/CLU1262" ) 
fwrite( resumen_variables, 
        file= "resumen_variables.txt",
        sep= "\t" )
#write.csv(resumen_variables, filename="D:/OneDrive/Personal/Maestria Data Mining/economia_finanzas/exp/CLU1262/resumen_variables.csv")
